...
│  │ 名称              │ 账号              │ 密码      │ 操作      │      │
│  ├──────────────────┼──────────────────┼──────────┼──────────┤      │
│  │ GitHub           │ user@email.com   │ •••••••• │ 👁 📋 ✏️ 🗑 │      │
│  │ Google           │ user@gmail.com   │ •••••••• │ 👁 📋 ✏️ 🗑 │      │
│  │ Stack Overflow   │ dev_user         │ •••••••• │ 👁 📋 ✏️ 🗑 │      │
│  └──────────────────┴──────────────────┴──────────┴──────────┘      │
│                                                                     │
│  每页 [ 20 条 ▾ ] │ [⏮] [◀] 第 1 / 1 页 [▶] [⏭] │ 共 3 条记录（当前显示 1-3） │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
...
| 3.3 | 密码列表展示 | 表格形式分页展示当前页密码条目 | ✅ |
| 3.4 | 新增/编辑/删除 | 密码的增删改操作 | ✅ |
| 3.5 | 查看/复制密码 | 解密显示，一键复制 | ✅ |
| 3.6 | 随机密码生成 | 可配置长度和字符类型的密码生成器 | ✅ |
| 3.7 | 搜索功能 | 按名称搜索 | ✅ |
| 3.8 | 修改主密码 | 验证旧密码后修改，重新加密所有条目 | ✅ |
| 3.9 | 分页展示 | 默认每页 20 条，可选 10/20/30/50，支持首页/上一页/下一页/末页，搜索与锁定后回到第一页；新增/导入的条目按名称排序后可能位于其他页 | ✅ |
...
- `src/plugins/password_manager/ui.rs` — UI 渲染（主密码界面、密码列表与分页、表单、生成器、导入导出、修改主密码）
- `Cargo.toml` — 新增 `aes-gcm`、`pbkdf2`、`hmac`、`sha2`、`rand`、`serde`、`csv` 依赖

**测试覆盖：** 25 个单元测试全部通过（加密解密、错误密钥验证、密码生成、密钥派生确定性、主密码哈希验证、JSON 导出导入往返、CSV 导出导入往返、分页总页数/页码收敛/分页范围/每页条数默认值与切换、条目变化后页码收敛、逐页渲染表格不出错）
