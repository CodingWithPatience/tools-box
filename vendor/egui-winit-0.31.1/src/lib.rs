...
fn is_copy_command(modifiers: egui::Modifiers, keycode: egui::Key) -> bool {
    keycode == egui::Key::Copy
        || (modifiers.command
            && keycode == egui::Key::C
            // Windows 终端需要收到带修饰键的 Key 事件，以区分 Ctrl+C 与 Ctrl+Shift+C。
            && !(cfg!(target_os = "windows") && modifiers.ctrl && modifiers.shift))
...
fn is_paste_command(modifiers: egui::Modifiers, keycode: egui::Key) -> bool {
    keycode == egui::Key::Paste
        || (modifiers.command
            && keycode == egui::Key::V
            // Windows 终端需要先处理 Ctrl+Shift+V，再显式请求平台读取剪贴板。
            && !(cfg!(target_os = "windows") && modifiers.ctrl && modifiers.shift))
...
mod tests {
    ...
    #[test]
    fn windows_ctrl_shift_c_is_preserved_as_key_event() {
        let is_native_copy = is_copy_command(command_modifiers(true), egui::Key::C);
        assert_eq!(is_native_copy, !cfg!(target_os = "windows"));
        assert!(is_copy_command(command_modifiers(false), egui::Key::C));
        assert!(is_copy_command(egui::Modifiers::default(), egui::Key::Copy));

        let ctrl_insert = egui::Modifiers {
            ctrl: true,
            ..Default::default()
        };
        assert_eq!(
            is_copy_command(ctrl_insert, egui::Key::Insert),
            cfg!(target_os = "windows")
        );
    }

    #[test]
    fn windows_ctrl_shift_v_is_preserved_as_key_event() {
        let is_native_paste = is_paste_command(command_modifiers(true), egui::Key::V);
        assert_eq!(is_native_paste, !cfg!(target_os = "windows"));
        assert!(is_paste_command(command_modifiers(false), egui::Key::V));
        assert!(is_paste_command(
            egui::Modifiers::default(),
            egui::Key::Paste
        ));

        let shift_insert = egui::Modifiers {
            shift: true,
            ..Default::default()
        };
        assert_eq!(
            is_paste_command(shift_insert, egui::Key::Insert),
            cfg!(target_os = "windows")
        );
    }
}
