...
impl Grid {
    ...
    // Tools Box patch: 滚动偏移超过屏幕行数时，原实现会出现无符号下溢
    // （debug 构建 panic，release 构建回绕），历史内容无法正常滚动。
    // 现在把窗口在「历史行 + 屏幕行」上滑动并截取屏幕行数，行为在两种构建下一致。
    ...
    // Tools Box patch: 暴露当前已缓存的历史行数，供调用方计算滚动条范围。
