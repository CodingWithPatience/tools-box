# 临时笔记插件设计方案

## 一、功能需求

| 功能 | 说明 | 优先级 |
|------|------|--------|
| 目录分类 | 支持创建多级目录，按目录组织笔记 | P0 |
| 笔记管理 | 创建、编辑、删除笔记 | P0 |
| Markdown 支持 | 支持 Markdown 格式编辑和预览 | P0 |
| 搜索功能 | 按标题和内容搜索笔记 | P1 |
| 收藏功能 | 标记重要笔记为收藏 | P1 |
| 标签功能 | 为笔记添加标签分类 | P1 |
| 排序功能 | 按创建时间、更新时间、标题排序 | P2 |
| 导入导出 | 批量导入导出笔记 | P2 |

---

## 二、界面设计

### 2.1 主界面布局

```
┌─────────────────────────────────────────────────────────────────────┐
│  📝 临时笔记                [ + 新建笔记 ] [ + 新建目录 ] [ 🔍 搜索 ] │
├──────────────┬──────────────────────────────────────────────────────┤
│              │                                                      │
│  目录树：     │  标题：[ 我的笔记标题                               ] │
│  ┌────────┐  │                                                      │
│  │ 📁 全部  │  │  [ Markdown 编辑 ] [ 预览 ]                         │
│  │ 📁 收藏  │  │  ┌──────────────────────────────────────────────┐ │
│  │ 📁 工作  │  │  │                                              │ │
│  │  ├─ 会议  │  │  │  # 标题                                       │ │
│  │  └─ 待办  │  │  │                                              │ │
│  │ 📁 学习  │  │  │  这是一段 **Markdown** 文本...               │ │
│  │  ├─ Rust  │  │  │                                              │ │
│  │  └─ 前端  │  │  │                                              │ │
│  │ 📁 个人  │  │  │                                              │ │
│  └────────┘  │  │  └──────────────────────────────────────────────┘ │
│              │                                                      │
│  最近笔记：   │  目录：[📁 工作 > 会议 ▼]                           │
│  ┌────────┐  │  标签：[Rust] [会议] [+]                            │
│  │ 会议记录  │                                                      │
│  │ 学习笔记  │  [ 💾 保存 ] [ 🗑 删除 ] [ ⭐ 收藏 ]                │
│  │ ...      │                                                      │
│  └────────┘  │                                                      │
│              │                                                      │
├──────────────┴──────────────────────────────────────────────────────┤
│  共 12 条笔记 | 最后更新：2024-01-15 10:30                          │
└─────────────────────────────────────────────────────────────────────┘
```

### 2.2 目录管理弹窗

```
┌───────────────────────────────────────┐
│  目录管理                    [ ✕ ]    │
├───────────────────────────────────────┤
│                                       │
│  目录名称：[ 新目录名              ]   │
│  上级目录：[ 根目录 ▼ ]               │
│                                       │
│  现有目录：                            │
│  ┌─────────────────────────────────┐ │
│  │ 📁 工作                         │ │
│  │   📁 会议                       │ │
│  │   📁 待办                       │ │
│  │ 📁 学习                         │ │
│  │   📁 Rust                       │ │
│  │   📁 前端                       │ │
│  └─────────────────────────────────┘ │
│                                       │
│  [ 确定 ] [ 取消 ] [ 删除选中目录 ]   │
│                                       │
└───────────────────────────────────────┘
```

---

## 三、数据结构设计

### 3.1 核心数据结构

```rust
/// 目录结构
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NoteFolder {
    pub id: i64,
    pub name: String,
    pub parent_id: Option<i64>,  // None 表示根目录
    pub sort_order: i32,
    pub created_at: String,
}

/// 笔记条目
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NoteEntry {
    pub id: i64,
    pub title: String,
    pub content: String,
    pub folder_id: Option<i64>,  // None 表示未分类
    pub is_favorite: bool,
    pub tags: Vec<String>,
    pub created_at: String,
    pub updated_at: String,
}

/// 笔记表单（编辑用）
#[derive(Debug, Clone)]
pub struct NoteForm {
    pub title: String,
    pub content: String,
    pub folder_id: Option<i64>,
    pub tags: String,  // 逗号分隔
}

/// 排序方式
#[derive(Debug, Clone, PartialEq)]
pub enum SortBy {
    CreatedAt,
    UpdatedAt,
    Title,
}

/// 视图模式
#[derive(Debug, Clone, PartialEq)]
pub enum NoteViewMode {
    Edit,    // 编辑模式
    Preview, // Markdown 预览
    Split,   // 左侧编辑、右侧 Markdown 预览
}
```

---

## 四、数据库设计

```sql
-- 笔记目录
CREATE TABLE note_folders (
    id         INTEGER PRIMARY KEY AUTOINCREMENT,
    name       TEXT NOT NULL,
    parent_id  INTEGER,
    sort_order INTEGER DEFAULT 0,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (parent_id) REFERENCES note_folders(id) ON DELETE CASCADE
);

-- 笔记条目
CREATE TABLE note_entries (
    id         INTEGER PRIMARY KEY AUTOINCREMENT,
    title      TEXT NOT NULL,
    content    TEXT NOT NULL DEFAULT '',
    folder_id  INTEGER,
    is_favorite BOOLEAN DEFAULT FALSE,
    tags       TEXT DEFAULT '',  -- 逗号分隔的标签
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (folder_id) REFERENCES note_folders(id) ON DELETE SET NULL
);

CREATE INDEX idx_note_entries_folder ON note_entries(folder_id);
CREATE INDEX idx_note_entries_favorite ON note_entries(is_favorite);
CREATE INDEX idx_note_entries_updated ON note_entries(updated_at);
```

---

## 五、目录结构

```
src/plugins/note_taker/
├── mod.rs          # 插件入口
├── ui.rs           # UI 渲染逻辑
├── models.rs       # 数据结构定义
├── store.rs        # 数据库 CRUD 操作
└── markdown.rs     # Markdown 渲染
```

---

## 六、实现步骤

| 步骤 | 任务 | 说明 | 状态 |
|------|------|------|------|
| 1 | 创建插件骨架 | 目录结构、Plugin trait 实现 | ✅ |
| 2 | 数据库初始化 | 创建 note_folders 和 note_entries 表 | ✅ |
| 3 | 实现目录管理 | 目录树展示、新增/编辑/删除目录 | ✅ |
| 4 | 实现笔记列表 | 按目录筛选笔记列表 | ✅ |
| 5 | 实现笔记编辑 | 标题、内容编辑器 | ✅ |
| 6 | 实现 Markdown 预览 | 使用 pulldown-cmark 解析，并按阶段完善文档模型和 egui 渲染 | 🚧 阶段 1 已完成 |
| 7 | 实现搜索功能 | 按标题和内容搜索 | ✅ |
| 8 | 实现收藏功能 | 收藏/取消收藏 | ✅ |
| 9 | 实现标签功能 | 添加/删除标签 | ✅ |
| 10 | 实现排序功能 | 按时间/标题排序 | ✅ |
| 11 | 注册插件 | 在 plugins/mod.rs 中注册 | ✅ |

---

## 七、依赖库

```toml
[dependencies]
pulldown-cmark = "0.13.4"  # Markdown 解析渲染
```

其他使用现有依赖：
- `rusqlite`: SQLite 数据库
- `serde/serde_json`: 序列化
- `anyhow`: 错误处理

---

## 八、技术要点

1. **目录树实现**：使用递归方式渲染目录树，支持无限层级
2. **Markdown 渲染**：使用 `pulldown-cmark` 解析 Markdown，先转换为内部文档模型，再通过 egui 渲染
   - `MarkdownDocument` 保存块级节点
   - `MarkdownBlock` 表示标题、段落、引用、列表、代码块、表格和分割线
   - `MarkdownInline` 表示文本、嵌套样式、行内代码、链接、图片和换行
   - 原始 HTML 以文本形式保留，不执行或解释 HTML
   - 解析与 UI 渲染分离，为后续主题、交互和缓存优化提供基础
3. **标签管理**：标签存储为逗号分隔字符串，解析为 Vec 显示
4. **延迟操作模式**：使用 UiAction 枚举避免 egui 渲染期间的借用冲突

---

## 九、产出文件

| 文件 | 说明 |
|------|------|
| `src/plugins/note_taker/mod.rs` | 插件入口，Plugin trait 实现 |
| `src/plugins/note_taker/ui.rs` | UI 渲染逻辑（目录树、笔记编辑器） |
| `src/plugins/note_taker/models.rs` | 数据结构定义 |
| `src/plugins/note_taker/store.rs` | 数据库 CRUD 操作 |
| `src/plugins/note_taker/markdown.rs` | Markdown 解析模型和 egui 渲染 |
| `src/plugins/mod.rs` | 注册新插件 |

---

## 十、测试计划

- [x] 目录创建、编辑、删除正常
- [x] 目录树层级显示正确
- [x] 笔记创建、编辑、删除正常
- [x] 笔记按目录筛选正确
- [x] Markdown 预览渲染正确
- [x] 搜索功能正常
- [x] 收藏功能正常
- [x] 标签添加删除正常
- [x] 排序功能正常

---

## 十一、Markdown 预览分阶段实施记录

### 阶段 1：解析中间模型与核心语义（已完成）

实现文件：`src/plugins/note_taker/markdown.rs`

已完成内容：

- 增加 `MarkdownDocument`、`MarkdownBlock`、`MarkdownListItem` 和 `MarkdownInline` 模型
- 使用事件树将 `pulldown-cmark` 事件转换为拥有所有权的内部模型
- 支持标题、段落、嵌套粗体/斜体/删除线、行内代码、换行和分割线
- 支持有序列表、无序列表、嵌套列表和任务列表状态
- 支持引用块、表格结构和列对齐信息、链接、图片和代码块语言标识
- 链接当前保留目标并使用超链接样式，跳转交互留待后续阶段；图片当前显示带地址的占位文本
- 原始 HTML 按文本保留，避免渲染层执行 HTML
- 现有 `MarkdownRenderer` 已改为消费 `MarkdownDocument`
- 增加解析模型单元测试，覆盖嵌套样式、列表、任务列表、表格、链接、图片、代码语言、引用和分割线

验证方式：

```text
rustfmt --edition 2024 --check src/plugins/note_taker/markdown.rs
cargo test
cargo build
```

说明：仓库级 `cargo fmt --check` 当前还会报告其他既有文件的格式差异，本阶段未修改这些无关文件。

### 阶段 2：预览视觉样式（已完成）

本阶段只处理渲染表现，不改变已完成的解析模型：

- 已完成：表格收集统一的行列边界后绘制共享网格线、单元格内边距和表头背景，颜色跟随 egui 当前主题；不使用相邻单元格重复边框
- 已完成：统一标题、段落、列表、分割线和块级内容间距
- 已完成：使用 egui 主题颜色适配浅色和深色主题
- 已完成：优化引用、表格、行内代码和代码块容器；引用框占满预览区域整行，引用线贯穿整个引用框，引用框上下内边距保持对称
- 已完成：为空文档显示“暂无可预览内容”提示
- 已完成：行内内容统一使用 `LayoutJob` 渲染，软换行不再通过独立控件插入，避免换行后的伪缩进
- 已完成：普通文本使用 Body 字体的实际字号，不将字体行高误作为字号

阶段 2 验证通过后，下一步进入阶段 3：代码块增强。

### 阶段 3：代码块增强（已完成）

实现文件：`src/plugins/note_taker/markdown.rs`，复用 `src/utils/highlight.rs` 的 `SyntaxHighlighter`。

已完成内容：

- 使用代码块语言标识接入现有 `syntect` 高亮器，未知语言自动回退为纯文本
- 代码块改为只读 `LayoutJob` 渲染，不再使用可编辑的 `TextEdit`
- 增加“复制”按钮，通过 egui 输出命令复制代码原文
- 代码块内容使用水平滚动区域承载，长代码行不会被强制折行
- 按代码内容、语言、主题和等宽字号缓存高亮结果，缓存最多保留 128 条，减少重复解析并避免无界增长

验证方式：

```text
rustfmt --edition 2024 --check src/plugins/note_taker/markdown.rs
cargo test
cargo build
cargo build --release
```

验证结果：Markdown 模块 9 个测试通过，Debug 构建通过；标准 Release 输出目录中的可执行文件曾被占用，因此使用独立输出目录完成 Release 构建通过。最终全量测试通过（100 个通过、1 个忽略）。

阶段 3 实现完成后，下一步进入阶段 4：编辑与预览交互。

### 阶段 4：编辑与预览交互（已完成）

实现文件：`src/plugins/note_taker/ui.rs`、`src/plugins/note_taker/models.rs`

- 已完成：增加“编辑”“预览”“分栏”三种视图模式，分栏模式左右显示 Markdown 编辑器和预览区域
- 已完成：编辑内容变化后实时使用当前表单内容渲染预览，不引入额外的保存操作依赖
- 已完成：编辑和预览模式切换时同步纵向滚动位置；分栏模式由源码栏滚动变化触发预览栏跟随，预览栏保留独立滚动条但不反向修改源码位置
- 已完成：增加“复制 Markdown”操作，复制当前表单中的 Markdown 原文
- 已完成：切换笔记时重置视图滚动位置，避免沿用上一条笔记的滚动状态
- 已修复：代码块内部水平滚动区只关闭水平方向自动收缩，垂直方向按实际代码高度收缩，避免首个代码块占满分栏剩余高度并阻塞外层滚动
- 已修复：目录选择器嵌入预览区域；预览和分栏右侧预览使用独立背景、内边距和边框，避免工具行与预览栏视觉上连成一个区域

验证方式：

```text
rustfmt --edition 2024 --check src/plugins/note_taker/models.rs src/plugins/note_taker/ui.rs
cargo test
cargo build
cargo build --release
```

验证结果：阶段 4 修复滚动状态后，新增 UI 状态测试 2 个通过；最终全量测试 102 个通过、1 个忽略，Debug 构建和独立输出目录的 Release 构建通过。Release 构建仍有 API Tester 模块既有的 7 个未使用代码警告。

本次预览区域布局修复验证：全量测试 153 个通过、1 个忽略，Debug 构建和独立输出目录的 Release 构建通过。

### 阶段 5：性能与扩展（已完成）

- 已完成：MarkdownRenderer 按 Markdown 原文缓存最近一次 `MarkdownDocument`，内容未变化时复用解析结果，内容变化时重新解析
- 已完成：行内 `LayoutJob` 缓存纳入预览可用宽度、Body/Monospace 字号和关键主题颜色；主题或窗口宽度变化时使用新的渲染上下文重新计算布局
- 已完成：预览和分栏模式增加 Markdown 标题目录选择器，支持 H1～H6 及引用/列表中的嵌套标题，选择目录项后滚动到对应标题
- 已完成：标题工具栏保持单行布局，使用“复制”“导出”按钮并为操作按钮预留 280px 空间；标题目录使用自定义弹出层，按当前窗口可视高度的一半限制最大高度，标题较少时按实际内容高度显示，并根据上下可视空间选择弹出方向；预览与分栏模式使用独立的下拉状态
- 已完成：增加 Markdown 图片加载，支持相对路径、本地 `file://` 和远程 HTTP(S) 图片；图片读取与解码在后台线程执行，限制单张图片最大 20 MB、解码尺寸最大 8192×8192、缓存最多 64 项/64 MB、并发最多 8 项，并按预览宽度保持比例显示
- 已完成：增加“导出 Markdown”操作，通过文件选择器异步保存当前编辑内容，写入超时 10 秒；自动清理 Windows 文件名中的非法字符、控制字符和保留设备名，并补全 `.md` 扩展名

阶段 5 已完成项验证方式：

```text
rustfmt --edition 2024 --check src/plugins/note_taker/markdown.rs
cargo test note_taker::markdown::tests
cargo test note_taker::ui::tests
cargo test
cargo build
cargo build --release --target-dir target/codex-release-verify
```

阶段 5 已完成项验证结果：Markdown 模块 16 个测试通过，界面模块 3 个测试通过；覆盖解析缓存、主题/宽度布局缓存、标题目录提取、预览定位、图片地址规范化和导出文件名清理。

阶段 5 后续可选增强：图片缓存淘汰策略、导出时的图片资源打包，以及更完整的 Markdown 扩展语法支持。

后续实施必须从本节最后一个已完成阶段继续，并在对应阶段完成后同步更新状态、实现文件、测试命令和下一阶段入口。
