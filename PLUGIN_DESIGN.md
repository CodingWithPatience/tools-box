...
#### 1.2.2 Split Layout 差异显示模式（类 GitHub，固定行号 + 横向滚动）
...
│  原始文本 (左侧)              │  对比文本 (右侧)                      │
├─────────────────────────────┼───────────────────────────────────────┤
│ ┌行号┐ ┌──ScrollArea──────┐ │ ┌行号┐ ┌──ScrollArea──────┐         │
│ │ 1  │ │ function hello() │ │ │ 1  │ │ function hello() │         │
│ │ 2  │ │   console.log()  │ │ │ 2  │ │   console.log()  │         │
│ │ 3  │ │   return true ←──┼─│─│────│─│→ (红色背景)       │         │
│ │    │ │                  │ │ │ 3  │ │   console.log()  │         │
│ │ 4  │ │   } ←────────────┼─│─│────│─│→ (红色背景)       │         │
│ │    │ │                  │ │ │ 4  │ │   return false   │         │
...
说明：
- 左右两侧各有一个固定行号面板（灰色竖线分隔），不随水平滚动移动
- 内容区域使用 ScrollArea::both() 支持横向和纵向滚动
- 左侧红色背景标记删除的行，右侧绿色背景标记新增的行
- 左右两侧垂直滚动位置同步，保持内容行对齐
- 修改的行配对显示：左侧旧行 + 右侧新行，字符级差异高亮
```
...
| 1.4.16 | 编辑区行号 | 编辑区左侧固定行号面板，通过读取 ScrollArea 偏移实现垂直同步，不随水平滚动 | ✅ |
| 1.4.17 | Split 视图固定行号 | Split 视图左右栏各自独立固定行号面板，不随水平滚动 | ✅ |
